...
class BasicDetailsBloc extends Bloc<BasicDetailsEvent, BasicDetailsState> {
  final BasicDetailsService _service;
  final Logger _logger;

  BasicDetailsBloc({
    required BasicDetailsService service,
    required Logger logger,
  })  : _service = service,
        _logger = logger,
        super(BasicDetailsInitial()) {
    on<FetchBasicDetails>(_onFetchBasicDetails);
    on<SaveBasicDetails>(_onSaveBasicDetails);
  }

  /// Handles the fetch basic details event
  Future<void> _onFetchBasicDetails(
      FetchBasicDetails event, Emitter<BasicDetailsState> emit) async {
    emit(BasicDetailsLoading());
    try {
      final basicDetails = await _service.loadBasicDetails(event.eventId);
      if (basicDetails != null) {
        emit(BasicDetailsLoaded(basicDetails));
      } else {
        emit(const BasicDetailsError("Basic details not found."));
      }
    } catch (e) {
      _logger.e("Failed to fetch basic details for event ID: ${event.eventId}. Error: $e");
      emit(const BasicDetailsError("Failed to load basic details. Please try again."));
    }
  }

  /// Handles the save basic details event
  Future<void> _onSaveBasicDetails(
      SaveBasicDetails event, Emitter<BasicDetailsState> emit) async {
    emit(BasicDetailsSaving());
    try {
      // Load the existing Event using the eventId
      final existingEvent = await _service.loadEvent(event.basicDetails.eventId);
      if (existingEvent == null) {
        emit(const BasicDetailsError("Existing event not found for saving."));
        return;
      }

      // Save the updated details
      await _service.saveBasicDetails(event.basicDetails, existingEvent);
      emit(BasicDetailsSaved());
    } catch (e) {
      _logger.e("Failed to save basic details for event ID: ${event.basicDetails.eventId}. Error: $e");
