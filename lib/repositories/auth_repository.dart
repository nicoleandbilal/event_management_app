...
class AuthRepository {
  ...
  Future<User?> signInWithEmail(String email, String password) async {
    try {
      ...
      _logger.i(
          'AuthRepository: Sign in successful for user: ${userCredential.user?.email}');
  ...
  Future<User?> registerWithEmail(String email, String password) async {
    try {
      _logger.i('AuthRepository: Attempting to sign up with email: $email');
      final userCredential = await _firebaseAuth.createUserWithEmailAndPassword(
        email: email,
        password: password,
      );
      _logger.i(
          'AuthRepository: Sign up successful for user: ${userCredential.user?.email}');
      return userCredential.user;
    } on FirebaseAuthException catch (e) {
      _logger.e(
          'AuthRepository: FirebaseAuthException: ${e.code} - ${e.message}');
